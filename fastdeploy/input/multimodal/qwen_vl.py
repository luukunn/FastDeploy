...
"""Qwen-family (qwen_vl / qwen3_vl) multimodal processor."""
...
class QwenVLProcessor(MMProcessor):
    """Multimodal processor for Qwen2.5-VL and Qwen3-VL models.

    Implements the Qwen-family encoding logic: 3xN ndarray position IDs,
    fps tracking, and the standard Qwen vision position computation.
    """

    # ------------------------------------------------------------------
    # Class attributes (from mm_model_config QWEN_VL)
    # ------------------------------------------------------------------
    ...
    # Qwen3 overrides these; Qwen2.5 leaves them as None
    _video_min_pixels = None
    _video_max_pixels = None
    ...
    def add_video(self, frames, outputs, uuid, token_len=None, meta=None):
        preprocess_kwargs = {}
        if self._video_min_pixels is not None:
            preprocess_kwargs["min_pixels"] = self._video_min_pixels
            preprocess_kwargs["max_pixels"] = self._video_max_pixels
