...
"""Ernie4.5-VL multimodal processor."""
...
class ErnieVLProcessor(MMProcessor):
    """Multimodal processor for Ernie4.5-VL models.

    Implements Ernie-specific encoding logic: list-of-lists position IDs,
    boundary tokens (IMAGE_START/END, VIDEO_START/END), and independent
    3D position computation.
    """

    # ------------------------------------------------------------------
    # Class attributes (from mm_model_config ERNIE4_5_VL)
    # ------------------------------------------------------------------
    ...
    _default_fps = 2.0
    _default_min_frames = 16
    _default_max_frames = 180
    _default_target_frames = -1

    _supports_prompt_token_ids = True
    ...
    # ------------------------------------------------------------------
    # Write-back hook (Ernie preserves original prompt_token_ids)
    # ------------------------------------------------------------------
    def _write_back(self, request, outputs):
        """Ernie: preserve the original prompt_token_ids if already present.

        Unlike Qwen which always overwrites prompt_token_ids with the processed
        input_ids, Ernie keeps the original prompt_token_ids (from apply_chat_template)
        because the scheduler needs the original token count for slot allocation.
        """
    ...
    # ------------------------------------------------------------------
    # 3D position computation (Ernie-specific)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Max tokens per item (for scheduler)
    # ------------------------------------------------------------------
